      ...
      <div class="mrc-header">
        <div>
          ...
          <div class="mrc-meta" style="margin-top:5px;">
            ...
            <span>✅ ${ans.length}/${r.answers?.length||0} câu</span>
          </div>
        </div>
        ${sb}
      </div>
...
function showDetail(id){
  ...
  Object.entries(secs).forEach(([sec,qs])=>{
    ...
    qs.forEach(q=>{const opt=q.value>0?LIKERT_OPTS.find(l=>l.v===q.value):null;const disp=q.value===0?'⓪ KSD':q.value===null?'—':`${opt?.e||''} ${'★'.repeat(q.value)}${'☆'.repeat(5-q.value)} (${q.value})`;body+=`<div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid var(--surface2);font-size:12px;"><span style="color:var(--text2);flex:1;padding-right:8px">${q.code}. ${q.question}</span><span style="color:var(--warning);white-space:nowrap;font-weight:600">${disp}</span></div>`;});
    body+='</div>';
  });
